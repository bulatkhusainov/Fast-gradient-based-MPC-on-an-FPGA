...
Xint32 *x_hat_in_ptr_ddr = (Xint32 *)x_hat_IN_DEFINED_MEM_ADDRESS;
Xint32 *u_opt_out_ptr_ddr = (Xint32 *)u_opt_OUT_DEFINED_MEM_ADDRESS;
			...
			if (packet_type==1) //reset IP
						{
					...
					else if (packet_type==2) //start IP
							{
								...
								XFoo_Set_byte_x_hat_in_offset(&xcore,x_hat_IN_DEFINED_MEM_ADDRESS);
								XFoo_Set_byte_u_opt_out_offset(&xcore,u_opt_OUT_DEFINED_MEM_ADDRESS);
					...
					else if (packet_type==3) //write data DDR
					{
						...
						switch (packet_internal_ID)
						{
							case 0: //x_hat_in
							if (DEBUG)
									printf("write x_hat_in\n\r");

							if (FLOAT_FIX_X_HAT_IN==1) {
								for (i=0; i<ETH_PACKET_LENGTH-2; i++)
								{
									tmp_float=*(float*)&inputvec[i];
									inputvec_fix[i]=(int32_t)(tmp_float*pow(2,X_HAT_IN_FRACTIONLENGTH));
								}
								memcpy(x_hat_in_ptr_ddr+(ETH_PACKET_LENGTH-2)*packet_internal_ID_offset,inputvec_fix,(ETH_PACKET_LENGTH-2)*4);
							} else { //floating-point
								memcpy(x_hat_in_ptr_ddr+(ETH_PACKET_LENGTH-2)*packet_internal_ID_offset,inputvec,(ETH_PACKET_LENGTH-2)*4);
							}
							break;
					...
					else if (packet_type==4) //read data from DDR
					{
						...
						switch (packet_internal_ID)
						{
							case 0: //u_opt_in
							if (DEBUG)
								printf("read u_opt_out\n\r");
							memcpy(outvec_fix,u_opt_out_ptr_ddr+(ETH_PACKET_LENGTH_RECV-2)*packet_internal_ID_offset,(ETH_PACKET_LENGTH_RECV-2)*4);
							for (i=0; i<ETH_PACKET_LENGTH_RECV-2; i++)
							{
								if (FLOAT_FIX_U_OPT_OUT==1) { //fixed-point
									tmp_float=((float)outvec_fix[i])/pow(2,U_OPT_OUT_FRACTIONLENGTH);
									outvec[i]=*(Xint32*)&tmp_float;
								} else { //floating point
