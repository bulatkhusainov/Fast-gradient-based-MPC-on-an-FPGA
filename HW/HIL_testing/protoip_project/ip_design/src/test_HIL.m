...
for k = 1:size(sim_par.x_hat,2) % perform simulations was several initial conditions
    x_hat_in = (sim_par.x_hat(1:end,k))'; % initial condition is defined in generate_code.m
    ...
    for i=1:N_sim
        ...
        if (i < N_sim) || (Ts_last < 1e-10)
            x_hat_prev = x_hat_in;
            x_hat_in = (model.a*x_hat_in' + model.b*fpga_u_opt_out(1:current_design.m_inputs))';

            if system_settled == 0
                Ts_sim_fine = current_design.Ts;
                system_energy = norm(x_hat_in);
                while system_energy < sim_par.epsilon_settling
                    system_settled = 1;

                    model_remainder = c2d(model_c, Ts_sim_fine);
                    x_next_local = (model_remainder.a*x_hat_prev' + model_remainder.b*fpga_u_opt_out(1:current_design.m_inputs))';
             ...
             x_hat_in = (model_remainder.a*x_hat_in' + model_remainder.b*fpga_u_opt_out(1:current_design.m_inputs))';     
    ...
    % print the settling time
        ...
        if (TYPE_TEST==0)
            filename = strcat('../../ip_prototype/test/results/', project_name ,'/settling_time.dat');
        else
