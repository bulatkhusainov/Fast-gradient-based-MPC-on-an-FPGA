			...
			if (packet_type==1) //reset IP
						...
						switch (packet_internal_ID)
						{
							...
							if (FLOAT_FIX_X_IN_IN==1) {
								for (i=0; i<ETH_PACKET_LENGTH-2; i++)
								{
									...
									inputvec_fix[i]=(int32_t)(tmp_float*pow(2,X_IN_IN_FRACTIONLENGTH));
								}
								memcpy(x_in_in_ptr_ddr+(ETH_PACKET_LENGTH-2)*packet_internal_ID_offset,inputvec_fix,(ETH_PACKET_LENGTH-2)*4);
							} else { //floating-point
								memcpy(x_in_in_ptr_ddr+(ETH_PACKET_LENGTH-2)*packet_internal_ID_offset,inputvec,(ETH_PACKET_LENGTH-2)*4);
							}
							break;
					...
					else if (packet_type==4) //read data from DDR
					{
						...
						switch (packet_internal_ID)
						{
							case 0: //y_out_in
							if (DEBUG)
								printf("read y_out_out\n\r");
							memcpy(outvec_fix,y_out_out_ptr_ddr+(ETH_PACKET_LENGTH_RECV-2)*packet_internal_ID_offset,(ETH_PACKET_LENGTH_RECV-2)*4);
							for (i=0; i<ETH_PACKET_LENGTH_RECV-2; i++)
							{
								if (FLOAT_FIX_Y_OUT_OUT==1) { //fixed-point
									tmp_float=((float)outvec_fix[i])/pow(2,Y_OUT_OUT_FRACTIONLENGTH);
									outvec[i]=*(Xint32*)&tmp_float;
								} else { //floating point
