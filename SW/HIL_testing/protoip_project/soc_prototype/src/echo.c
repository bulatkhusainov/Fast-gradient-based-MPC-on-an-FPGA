...
float soc_x_hat_in[SOC_X_HAT_IN_VECTOR_LENGTH];

float soc_u_opt_out[SOC_U_OPT_OUT_VECTOR_LENGTH];
			...
			if (packet_type==1) //reset IP
						{
					...
					else if (packet_type==2) //start SOC function
							{
								...
								//Start SOC function
								soc_user(soc_x_hat_in,soc_u_opt_out);
					...
					else if (packet_type==3) //write data to SOC input arrays
					{
						...
						switch (packet_internal_ID)
						{
							case 0: //soc_x_hat_in
							if (DEBUG)
									printf("write soc_x_hat_in\n\r");

								for (i=0; i<ETH_PACKET_LENGTH-2; i++)
								{
									tmp_float=*(float*)&inputvec[i];
									if(i+(ETH_PACKET_LENGTH-2)*packet_internal_ID_offset > SOC_X_HAT_IN_VECTOR_LENGTH)
										{break;}
									soc_x_hat_in[i+(ETH_PACKET_LENGTH-2)*packet_internal_ID_offset] = tmp_float;
								}
							break;
					...
					else if (packet_type==4) //read data from SOC output vectors
					{
						...
						switch (packet_internal_ID)
						{
							...
							for (i=0; i<ETH_PACKET_LENGTH_RECV-2; i++)
							{
								if (i+(ETH_PACKET_LENGTH_RECV-2)*packet_internal_ID_offset > SOC_U_OPT_OUT_VECTOR_LENGTH)
									break;
								outvec[i]=*(Xint32*)&soc_u_opt_out[i+(ETH_PACKET_LENGTH_RECV-2)*packet_internal_ID_offset];
							}
							break;
